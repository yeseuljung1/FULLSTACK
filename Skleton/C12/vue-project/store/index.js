...
export default createStore({
    ...
    actions: {
        ...
        async updateTransactions({ commit }, payload) {
            try {
                await axios.put(`http://localhost:3000/transactions/${payload.date}`, payload.transactions);
    ...
    getters: {
        ...
        totalIncome: (state) => {
            ...
            for (const date in state.transactions) {
                total += state.transactions[date].filter((t) => t.type === 'income').reduce((sum, t) => sum + t.amount, 0);
            }
            return total;
        },
        totalExpense: (state) => {
            let total = 0;
            for (const date in state.transactions) {
                total += state.transactions[date].filter((t) => t.type === 'expense').reduce((sum, t) => sum + t.amount, 0);
            }
            return total;
        },
